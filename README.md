...
*   **Dual-Format Semantic Parsing (JSON & XML)**: Automatically detects XML responses and parses them dynamically into generic object structures. Evaluates tags, attributes (ignoring order variations), and inner values, fully utilizing the same semantic diff engine.
*   **Persistent Configuration Cache (LocalStorage)**: Automatically caches all input fields (Base URLs, Methods, Paths, Request Body, and manual Ignore lists). Reopening the app immediately restores your exact workspace state seamlessly.
...
2.  **Common Request Settings**:
    *   *Method*: `GET` (or `POST`)
    *   *Path*: `/api/v1/users` (or `/api/v1/xml` to test the new XML support!)
    *   *Body* (if using `POST`):
        *   JSON Body:
            ```json
            {
...
*   **JSON ＆ XML のデュアルフォーマット自動解析**: XMLレスポンスを自動検知し、内部でJSON互換の構造化ツリーに動的に変換します。タグの属性（順序の揺れを無視）、ネスト構造、および値を比較し、JSONと全く同じ高精度なセマンティック差分比較を可能にします。
*   **設定値の自動永続化（LocalStorageキャッシュ）**: 3環境のBase URL、リクエスト設定（Method, Path, Body）、および手動Ignoreパスリストをアプリ内で自動キャッシュします。アプリ再起動時でも前回の入力状態を瞬時に完全に自動復元し、ストレスフリーなデバッグ体験を提供します。
...
2.  **Common Request Settings (共通リクエスト)**:
    *   *Method*: `GET` (または `POST`)
    *   *Path*: `/api/v1/users` (または XML の検証用パス `/api/v1/xml` を指定！)
